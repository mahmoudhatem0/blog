  ...
  Route::get('/tags/{id}','websiteUIcontroller@tags')->name('tags.show');
   ...
   Route::get('/singelPost/{slug}','websiteUIcontroller@showPost')->name('aposts.showui');
     ...
     Route::get('/results', function () {
      $posts = App\Apost::where('title','like', '%' . request('search') . '%')->get();
        return view('results')
            ->with('posts', $posts )
            ->with('title', 'Result : ' . request('search') )
            ->with('query', request('search') )
            ->with('category',App\Category::all()->take(5) )
            ->with('tags',App\Tag::all())
            ->with('frist_post',App\Apost::orderBy('created_at','desc')->first())
            ->with('second_post',App\Apost::orderBy('created_at','desc')->skip(1)->take(1)->get()->first())
            ->with('third_post',App\Apost::orderBy('created_at','desc')->skip(2)->take(1)->get()->first())
            ->with('frist_category',App\Category::orderBy('created_at','desc')->first())
            ->with('second_category',App\Category::orderBy('created_at','desc')->skip(1)->take(1)->get()->first())
            ->with('third_category',App\Category::orderBy('created_at','desc')->skip(2)->take(1)->get()->first())
            ->with('four_category',App\Category::orderBy('created_at','desc')->skip(3)->take(1)->get()->first())
